...
const APP_CONFIG = {
    recursos_humanos: {
        ...
        fields: [
            { key: 'nombre', id: 'nombre-recursos_humanos' },
            { key: 'tipo_documento', id: 'tipo_documento-recursos_humanos' },
            { key: 'documento', id: 'documento-recursos_humanos' },
            { key: 'fecha_ingreso', id: 'fecha_ingreso-recursos_humanos' },
            { key: 'cargo', id: 'cargo-recursos_humanos' },
            { key: 'salario', id: 'salario-recursos_humanos' },
            { key: 'area', id: 'area-recursos_humanos' },
            { key: 'telefono', id: 'telefono-recursos_humanos' },
            { key: 'correo', id: 'correo-recursos_humanos' },
            { key: 'contrato', id: 'contrato-recursos_humanos' },
            { key: 'contacto', id: 'contacto-recursos_humanos' }
        ]
    },
    ausentismos: {
...
const EntityManager = {
    ...
    async saveEntity(tipo) {
        try {
            const config = APP_CONFIG[tipo];
            if (!config) {
                Utils.showMessage(`Configuración no encontrada para tipo: ${tipo}`, 'error');
                console.error('APP_CONFIG no tiene configuración para', tipo);
                return;
            }
            if (!config.fields) {
                Utils.showMessage(`No se definieron campos para tipo: ${tipo}`, 'error');
                console.error('No fields en APP_CONFIG para', tipo, config);
                return;
            }
            const form = document.getElementById(config.formId);
            if (!form) {
                Utils.showMessage(`Formulario no encontrado para tipo: ${tipo}`, 'error');
                console.error('No se encontró el formulario', config.formId);
                return;
            }
